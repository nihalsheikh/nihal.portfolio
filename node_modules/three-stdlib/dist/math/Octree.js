...
var Octree = function() {
  ...
  var _capsule = new Capsule.Capsule();
  ...
  Object.assign(Octree2.prototype, {
    ...
    split: function(level) {
      if (!this.box)
        return;
      var subTrees = [], halfsize = _v2.copy(this.box.max).sub(this.box.min).multiplyScalar(0.5), box, v, triangle;
      for (let x = 0; x < 2; x++) {
        for (let y = 0; y < 2; y++) {
          for (let z = 0; z < 2; z++) {
            box = new THREE.Box3();
    ...
    rayIntersect: function(ray) {
      if (ray.direction.length() === 0)
        return;
      var triangles = [], triangle, position, distance = 1e100, result;
